...
export class CooldownManager {
	...
	getCommandData(
		name: string,
		guildId?: string,
	): [name: string, data: CooldownProps | undefined] | undefined {
	...
	has(options: CooldownHasOptions): ReturnCache<boolean> {
		const [resolve, data] =
			this.getCommandData(options.name, options.guildId) ?? [];
		if (!(data && resolve)) return false;

		return fakePromise(
			this.resource.get(`${resolve}:${data.type}:${options.target}`),
		).then((cooldown) => {
	...
	set(options: CooldownSetOptions) {
		return this.resource.set(
			CacheFrom.Gateway,
			`${options.name}:${options.type}:${options.target}`,
			{
	...
	context(
		...
		use?: keyof UsesProps,
		guildId?: string,
	): ReturnCache<{ type: string; use: number | true } | true> {
		...
		target ??= context.author.id;
		return {
			use: this.use({ name: context.command.name, target, use, guildId }),
			type: target as 'user' | 'channel' | 'guild',
		};
	}
	...
	drip(options: CooldownDripOptions): ReturnCache<boolean | number> {
		...
		if (deltaMS >= options.props.interval) {
			return fakePromise(
				this.resource.patch(
					CacheFrom.Gateway,
					`${options.name}:${options.props.type}:${options.target}`,
					{
						lastDrip: now,
